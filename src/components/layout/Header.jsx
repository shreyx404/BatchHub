      ...
      <div className="mx-auto max-w-6xl px-4 h-14 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
              ...
              {searchOpen ? (
                <div className="flex items-center gap-2 animate-fade-in">
                  <input
                    ...
                    autoFocus
                    className="w-40 sm:w-56 h-9 px-3 rounded-lg bg-[var(--color-surface-2)] border border-[var(--color-border)] text-[var(--text-sm)] text-[var(--color-text)] placeholder:text-[var(--color-text-dim)] focus:outline-none focus:border-[var(--color-border-light)] transition-colors tracking-[0.005em]"
                  />
                  <button
          ...
          {!isAdmin && (
            <Link
              to="/admin"
              className="p-2 rounded-lg hover:bg-[var(--color-surface-2)] transition-colors text-[var(--color-text-dim)] hover:text-[var(--color-text-muted)]"
              aria-label="Admin"
            >
              <Settings size={18} />
            </Link>
