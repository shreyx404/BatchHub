...
- [x] **T-111** — "Editorial Elevated" UI Refinement & Mobile/Tablet Ergonomics: Comprehensive UI upgrade preserving the dark editorial identity while significantly elevating visual depth, rhythm, and information design. Implemented multi-tier surface elevation ladder (`#000000`, `#050505`, `#0A0A0A`, `#111111`, `#141414`), warm amber accent tokens (`--color-amber: #D4A574`, `--color-amber-dim`, `--color-amber-glow`), and oversized serif typography (`--text-6xl`, Playfair Display up to 80px desktop). Redesigned `HomePage` hero with subtle film grain overlay, thin amber rule, small-caps tracked tagline, and live stats counter (`LIVE` pulse, active deliverables, subjects). Upgraded `DeadlineBanner` with large typographic countdown cards (`DD:HH:MM`) with live ticking timer (`useCountdown`) and count-pulse animations. Introduced Bento grid layouts in `NoticesSection` and `PinnedSection` (lead item 2-column span with amber accents). Implemented continuous ledger timeline in `PostGrid` (`gap-0`). Elevated `PostCard` typography, amber hover glow, diamond pulse on urgent deadlines, and created date display. Differentiated `ArchivePage` with historical atmospheric styling (muted serif heading, subtle opacity). Enriched `Footer` with quick navigation links (Home, Calendar, Archive) with amber hover states and personality signature. Fully optimized and verified across mobile (375px), tablet (768px), and desktop (1536px) with 39/39 automated tests and zero build errors.
- [x] **T-081** — Dark / Light Theme Toggle & Editorial Paper Light Mode: Implemented complete dual-theme system honoring BatchHub's "Editorial Elevated" aesthetic. Built `ThemeContext.jsx` with persistent theme state (`'dark' | 'light' | 'system'`), `localStorage` persistence under `batchhub_theme`, dynamic system preference detection, and automated browser `<meta name="theme-color">` updates (`#050505` dark, `#F8F7F4` light). Added zero-flicker anti-flash script in `index.html` executing before DOM render. Expanded `index.css` with calibrated light mode tokens (`#F8F7F4` editorial linen background, `#FFFFFF` card surfaces, `#181716` deep charcoal typography, `#DCD7CE` hairline rules, and `#A86A24` amber ink with WCAG AAA/AA contrast). Built sharp `ThemeToggle.jsx` with rotating Sun/Moon micro-animations and square corners. Integrated toggle in main `Header.jsx`, `AdminPage.jsx` header bar, and `AdminLogin.jsx` screen. Updated `CalendarControls.jsx`, `CalendarGrid.jsx`, `CalendarWeekView.jsx`, `CalendarAgendaView.jsx`, `CalendarSidebar.jsx`, `FilterBar.jsx`, `ArchiveSortBar.jsx`, and `PostForm.jsx` to use dynamic theme variables and dedicated event chip classes. Verified end-to-end with zero build errors and browser subagent recording.
- [x] **T-112** — High-Contrast Text & Action Button Fix for Dual-Theme System: Resolved critical light-mode contrast defect where active filter pills, view switcher buttons (`MONTH`), calendar day cells (`09`), and primary action buttons (`+ New Post`, `+ New Deadline`) rendered as solid black blocks with unreadable black-on-black text due to Tailwind CSS v4 arbitrary CSS variable text property ambiguities. Replaced fragile inline arbitrary text utilities with bulletproof, first-class `.btn-primary` and `.badge-inverse` utility classes in `index.css` with cascading child color and SVG stroke enforcement. Updated `AdminPage`, `AdminCalendar`, `CalendarControls`, `CalendarGrid`, `CalendarWeekView`, `CalendarAgendaView`, `CalendarSidebar`, `FilterBar`, `ArchiveSortBar`, `PostTable`, `PostForm`, `SubjectManager`, `AdminLogin`, `PostPage`, and `NotFoundPage`. Verified with 39/39 passing automated tests and visual browser subagent screenshots in both Light and Dark modes.
- [x] **T-113** — Top Header College Study Material Button & Dynamic Admin Settings: Added a permanent button in the top navigation header positioned directly between **Archive** and the **Theme toggle**, linking students to the batch's centralized Google Drive repository (`https://drive.google.com/drive/folders/1MukhwWiRG7CTsOmi2a__buSqKY_HcQXy?usp=drive_link`). Created the `app_settings` table in Supabase (`schema.sql` with public read RLS and service-role write) and seeded default `college_material_url` in `seed.sql`. Added `DEFAULT_COLLEGE_MATERIAL_URL` and `SETTING_KEYS` in `constants.js`, and `DEMO_SETTINGS` in `demoData.js`. Implemented `fetchSetting`, `updateSetting`, `fetchCollegeMaterialUrl`, and `updateCollegeMaterialUrl` in `lib/api.js`. Added `getSetting` and `updateSetting` serverless actions in `api/admin.js` with field whitelisting and URL validation. Created `useCollegeMaterial.js` custom hook with module-level caching and cross-component custom event reactivity. Built `SettingsManager.jsx` admin interface (`/admin/settings`) with URL input, live URL validation, "Test Link" action, "Reset to Default" action, and real-time header placement preview. Added Settings link to `AdminSidebar.jsx` and Quick Action card to `AdminPage.jsx`. Verified with 45/45 passing automated unit & security tests, successful production build, and browser verification recording.
- [x] **T-114** — Dedicated Calendar Page Search & Top Bar Search Icon Removal: Removed the redundant search icon and expandable search input from the global top bar (`Header.jsx`) across all pages, simplifying the top navigation actions to Academic Calendar, Archive, College Study Material (Google Drive), Theme Toggle, and Admin Settings. Added a prominent, dedicated `SEARCH` button in `CalendarControls.jsx` within the action toolbar alongside `TODAY` and the View Mode switcher. Designed an expandable, responsive in-page search bar with auto-focus, live match count badge (`X matches`), quick clear `(X)` button, dismiss `Close` button, `Escape` key handler, and global `Ctrl+K` / `Cmd+K` keyboard shortcut support. Enhanced `CalendarPage.jsx` post filtering to search across deliverable titles, markdown content, tags, subject names, and subject codes. Verified with 45/45 passing automated tests, zero build errors, and verified with visual browser subagent testing.
- [x] **T-115** — Light Theme Optimization for Calendar Sidebar & Urgent Deliverables: Calibrated light mode design tokens in `index.css` for high readability (`--color-text-muted: #4A4640`, `--color-text-dim: #7A756D`, `--color-border: #D5CFBF`, `--color-border-light: #C2BAAA`, `--color-surface-hover: #DDD8CE`, light `.day-cell-selected` amber tint, and crisp light urgent/draft event chip borders and backgrounds). Replaced hardcoded dark hex colors in `CalendarSidebar.jsx` with dynamic CSS variables and dual-mode Tailwind variants (`dark:`), ensuring post descriptions, inspector cards, countdown badges (`⏳ In Xh`), and status pills (`URGENT`, `DRAFT`, `PAST DUE`, `ARCHIVED`) are fully legible in light theme. Added an urgent assignment demo fixture due today (`post-today` with `getTodayDeadline(23, 59)`) in `src/lib/demoData.js` to enable immediate visual testing of urgent chips, countdowns, and inspector cards in both themes. Verified with 45/45 automated tests and end-to-end browser subagent validation.
- [x] **T-116** — High-Contrast White Text for Urgent Deliverables in Calendar: Enhanced urgent deadline event chips (`.event-chip-urgent`, `CalendarGrid`, `CalendarWeekView`) to use crisp, bright white text (`#FFFFFF`) and `#FECACA` countdown text on dark red urgent backgrounds (`#180D0D` with `#EF4444` border) in dark mode. Added `@custom-variant dark (&:where([data-theme="dark"], [data-theme="dark"] *, .dark, .dark *));` in `index.css` for Tailwind CSS v4 selector-based theme matching.
- [x] **T-117** — Comprehensive Mobile & Tablet Layout Optimization for Calendar & Navigation: Replaced cramped 840px horizontal scrolling in `CalendarWeekView` on mobile & tablet portrait (< 1024px) with a sleek full-width 7-day selector strip (`grid grid-cols-7`) and vertical day-by-day timetable showing full-width cards, clear status badges, and comfortable touch targets (≥40–44px). Redesigned `CalendarControls` toolbar on mobile into a clean, un-wrapped 2-row layout (Row 1: Date Navigator + TODAY button; Row 2: View Mode Switcher + SEARCH button). Removed nested scroll trapping in `CalendarAgendaView` on mobile/tablet (`max-h-none overflow-visible lg:overflow-y-auto lg:max-h-[calc(100dvh-200px)]`). Refined `Header.jsx` icon sizes and spacing for 360px phones. Verified on desktop (1280x800), tablet (768x1024), and mobile (390x844).
- [x] **T-118** — Public Notes Gallery (Folder View): Build full-page `/notes` gallery linking to externally-hosted HTML study notes. Implement a folder-based UI where subjects act as folders. Clicking a folder navigates to the subject's notes (`/notes?subject=...`). Includes global debounced search overriding folders.
- [x] **T-119** — Admin Notes Manager: Build `/admin/notes` dashboard to CRUD notes (title, subtitle, subject, external URL). Enforce strict URL scheme validation (`http://`, `https://`). Integrated with `api/admin.js` serverless function and dual-mode memory fallback.
- [x] **T-120** — Notes Database Infrastructure: Create `notes` table in `supabase/schema.sql` with public read RLS, trigger-updated timestamp, and cascade-null subject relationships.
...
- [x] **T-124** — Admin Login Back to Home Navigation: Added a dedicated "← Back to BatchHub" navigation link (`<Link to="/">`) positioned in the top-left corner of the `AdminLogin.jsx` authentication screen with `ArrowLeft` icon, mobile-responsive label ("Back" on mobile, "Back to BatchHub" on desktop), and warm amber hover accent, eliminating dead-end navigation and allowing students and users to return effortlessly to the main homepage.
