...
| F-03 | **Dynamic Unpinning** | Posts with expired `pinned_until` duration or expired `due_date` automatically lose their pinned status client-side |
...
| F-06 | **Post Detail Page** | Full Markdown rendering, tags (if legacy), direct resource links & attachments in defined order, and formatted share button |
...
| F-08h | **Notes Gallery (Folder View)** | Dedicated full-page Notes section (`/notes`) replacing flat lists with an intuitive folder-based UI. Subjects act as folders containing externally-hosted HTML study notes. Features global debounced search spanning across all subjects. |
...
| F-09a | **Back to Home Navigation** | Dedicated top-left return navigation link ("← Back to BatchHub") on the admin login screen allowing students or users who navigate to `/admin` to smoothly return to the public homepage |
